...
const log = debug("neovate:file-paste");
...
/** Extract all files from a drop event — unrestricted type. */
function extractDropFiles(dataTransfer: DataTransfer): File[] {
  const files: File[] = [];
  for (let i = 0; i < dataTransfer.files.length; i++) {
    files.push(dataTransfer.files[i]);
  }
  log("extractDropFiles: result count=%d", files.length);
  return files;
}
 ...
 */
function extractImagesFromHtml(html: string): FileAttachment[] {
...
function handleFileReadError(err: unknown) {
  log("readFileAsAttachment failed: %s", err instanceof Error ? err.message : String(err));
}

export function createImagePasteExtension(onFiles: (files: FileAttachment[]) => void) {
  return Extension.create<FilePasteOptions>({
    name: "imagePaste",
        ...
        new Plugin({
          ...
          props: {
            handlePaste(_view, event) {
              ...
              // Clipboard paste only supports images (not arbitrary files)
            ...
            handleDrop(_view, event) {
              ...
              // Drop supports all file types (images, text files, PDFs)
              const dropFiles = extractDropFiles(dataTransfer);
              if (dropFiles.length === 0) {
                log("handleDrop: no files found");
                return false;
              }

              log("handleDrop: found %d files, preventing default", dropFiles.length);
              event.preventDefault();
              Promise.all(dropFiles.map(readFileAsAttachment))
                .then((attachments) => {
                  log(
